...
import { crearPaciente, actualizarPaciente, obtenerSeguros, obtenerPacientePorId } from '../services/pacienteService';
...
export default function PacienteForm() {
  ...
  const { register, handleSubmit, setValue, formState: { errors } } = useForm<PacienteCrear>();
      ...
      <div className="w-full max-w-3xl bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
        ...
        <div className="flex items-center gap-4 mb-8 border-b border-gray-100 pb-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              {esEdicion ? `Modificando registro #${id}` : 'Complete el formulario de ingreso'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* DNI */}
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700">DNI</label>
              <input 
                {...register("dni", { required: "El DNI es obligatorio", maxLength: 8, minLength: 8 })} 
                className="w-full p-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition"
                placeholder="8 dígitos"
              />
              {errors.dni && <span className="text-red-500 text-xs font-medium">{errors.dni.message || "Debe tener 8 dígitos"}</span>}
            </div>

            {/* Nombres */}
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700">Nombres</label>
              <input 
            ...
            {/* Apellidos */}
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700">Apellidos</label>
              <input 
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium mb-1 text-slate-700">Edad</label>
                    <input 
                        type="number" 
                ...
                <div>
                    <label className="block text-sm font-medium mb-1 text-slate-700">Sexo</label>
                    <select 
            ...
            {/* Fecha Nacimiento */}
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700">Fecha Nacimiento</label>
              <input 
                type="date" 
            ...
            {/* Seguro */}
            <div>
              <label className="block text-sm font-medium mb-1 text-slate-700">Seguro Médico</label>
              <select 
          ...
          <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
            ...
            <button 
                type="submit" 
                className="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2 font-medium shadow-lg shadow-blue-600/20 transition hover:-translate-y-0.5"
            >
              <Save size={18} /> 
